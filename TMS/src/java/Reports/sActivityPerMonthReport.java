 ...
 * @author Abree
 */
public class sActivityPerMonthReport extends HttpServlet {
 ...
 ******************************************************************************/
    if(_MyError.hasError()) {
        _ButtonClicked = " ";
        response.sendRedirect("/TMS/webpages/ReportPages/ActivitySummaryPerMonth.jsp");
    }
 ...
 ******************************************************************************/
    ...
    if(!_MyError.hasError()) {
        if(_ButtonClicked.compareToIgnoreCase("submit") == 0) {
            _MyReportData.setReportCode("ACTPMO");
        }
    }
        processRequest(request, response);
    }
